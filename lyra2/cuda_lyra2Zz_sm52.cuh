...
//	LD4S(last, rowInOut, 0, thread, threads);
	...
	LD4S(state1, rowInOut, 0, thread, threads);
...
void lyra2Zz_gpu_hash_32_1(uint32_t threads, uint32_t startNounce, uint2 *g_hash)
{
...
void lyra2Zz_gpu_hash_32_2(uint32_t threads, uint32_t startNounce, uint64_t *g_hash)
{
	const uint32_t thread = blockDim.y * blockIdx.x + threadIdx.y;

	if (thread < threads)
	{
		//printf("thread [%i] is yours", thread);
	...
	//}
		//rowa = WarpShuffle(state[0].x, 0, 4) & 7;
		reduceDuplexRowt_8_v2_l2zz(prev,iterator,rowa, state, thread, threads);
...
__global__
__launch_bounds__(64, 1)
void lyra2Zz_gpu_hash_32_3(uint32_t threads, uint32_t startNounce, uint2 *g_hash, uint32_t *resNonces)
{
	const uint32_t thread = blockDim.x * blockIdx.x + threadIdx.x;
...
namespace sm52 {
__global__ void lyra2Zz_gpu_hash_32_1(uint32_t threads, uint32_t startNounce, uint2 *g_hash) {}
__global__ void lyra2Zz_gpu_hash_32_2(uint32_t threads, uint32_t startNounce, uint64_t *g_hash) {}
__global__ void lyra2Zz_gpu_hash_32_3(uint32_t threads, uint32_t startNounce, uint2 *g_hash, uint32_t *resNonces) {}
} // end namespace sm52
