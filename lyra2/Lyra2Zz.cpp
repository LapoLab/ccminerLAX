...
typedef std::unique_ptr<l2zz_internal_data> l2zz_ptr_t;

struct l2zz_global_data { 
	static const size_t MAX_THREADS = MAX_GPUS;

	std::vector<l2zz_ptr_t> data_pool[MAX_THREADS];
	pthread_mutex_t data_pool_lock[MAX_THREADS];
	pthread_cond_t data_pool_conds[MAX_THREADS];
	pthread_mutex_t cond_locks[MAX_THREADS];

	pthread_mutex_t log_lock;

	std::stringstream logstream[MAX_THREADS];

	const size_t purge_threshold;
	const bool log;

	l2zz_global_data(void)
		:	purge_threshold(5),
			log(true)
	{
		for (size_t i = 0; i < MAX_THREADS; ++i) {
			pthread_mutex_init(&data_pool_lock[i], nullptr);
			pthread_cond_init(&data_pool_conds[i], nullptr);
			pthread_mutex_init(&cond_locks[i], nullptr);
		}
	...
	{
		for (size_t i = 0; i < MAX_THREADS; ++i) {
			pthread_mutex_destroy(&data_pool_lock[i]);
			pthread_cond_destroy(&data_pool_conds[i]);
		}

		pthread_mutex_destroy(&log_lock);
	}

	void flush_log(int thr_id)
	{
		std::string fname("C:\\ccminer_lyra2zz_");
		fname.append(std::to_string(thr_id));
		fname.append(".txt");

		FILE *f = fopen(fname.c_str(), "wb");

		if (f) {
			std::string str_logstream = logstream[thr_id].str();
			fwrite(str_logstream.c_str(), sizeof(str_logstream[0]), 
				str_logstream.size(), f);
		
			fclose(f);
		}

		logstream[thr_id].str("");
	}

	void lock(int thr_id)
	{
		pthread_mutex_lock(&data_pool_lock[thr_id]);
	}

	void unlock(int thr_id)
	{
		pthread_mutex_unlock(&data_pool_lock[thr_id]);
	}

	/* should only be called by lyra2Zz_submit function */
	void set_purge(int thr_id)
	{
		pthread_cond_signal(&data_pool_conds[thr_id]);
	}

	bool await_purge(int thr_id)
	{
		//struct timespec* t = 

		//pthread_cond_timedwait(&data_pool_conds[thr_id], &cond_locks[thr_id]
	}

	void internal_log(int thr_id, const char* format, ...)
	{
		const size_t buflen = 4096 << 2;
		...
		std::string timestr;
		{
			...
			strftime(&time_str[0], 19, "%H:%M:%S", tm_info);

			timestr.append(time_str);
		}

		va_list args;

		va_start(args, format);
		vsnprintf(buff, buflen, format, args);
		va_end(args);

		logstream[thr_id] << "\n[TH: " << thr_id << " | " << timestr << "]\t" << &buff[0] << "\n\n";
	}

	void free_internal_data(int thr_id, l2zz_internal_data *data)
	{
		lock(thr_id);

		/* check for purge first */
		if (data_pool[thr_id].size() >= purge_threshold) {
			data_pool[thr_id].clear();
		}

		for (const l2zz_ptr_t& ptr: data_pool[thr_id]) {
			if (ptr.get() == data) {
				internal_log(thr_id, LYRA2ZZ_LOG_HEADER "caught double free: %p", thr_id, data);
				goto out;
			}
		}

		{
			data_pool[thr_id].push_back(l2zz_ptr_t(data));
		}
out:
		unlock(thr_id);
	}
...
static std::unique_ptr<l2zz_global_data> g_data(new l2zz_global_data());
...
static atomic_flag g_shutdown;

static void l2zz_free_internal_data(int thr_id, lyra2zz_internal_t **data)
{
	if (g_shutdown.get())
		return;

	l2zz_internal_data *l2zz_internal = (l2zz_internal_data *)(*data);

	if (l2zz_internal) {
		g_data->free_internal_data(thr_id, l2zz_internal);
		*data = nullptr;
	}
}

static lyra2zz_internal_t * l2zz_deep_copy_internal_data(lyra2zz_internal_t *data)
{
	l2zz_internal_data *l2zz_internal = (l2zz_internal_data *) data;
	...
	l2zz_internal_data *ret = new l2zz_internal_data();

	*ret = *l2zz_internal;

	return (lyra2zz_internal_t *) ret;
}
...
static bool write_transactions_encoded(l2zz_internal_data *internal_data, std::stringstream& hex_data_stream)
{
	size_t num_trans = internal_data->transactions.size() & 0xFF;

	hex_data_stream << get_hexb((uint8_t) num_trans);

	size_t max_enc_bytes = 4096;
	char *encoded_t = (char *)malloc(max_enc_bytes);
	...
	for (size_t i = 0; i < internal_data->transactions.size(); ++i) {
		size_t hex_len = internal_data->transactions[i].size() << 1;
		...
		cbin2hex(
			&encoded_t[0],
			(const char *) internal_data->transactions[i].data(), 
			internal_data->transactions[i].size()
		);
...
#define l2zz_submit_fin(r)																		\
	do {																						\
		if (r) {																				\
			g_data->internal_log(work->l2zz.thr_id, LYRA2ZZ_LOG_HEADER "submit success");		\
		} else {																				\
			g_data->internal_log(work->l2zz.thr_id, LYRA2ZZ_LOG_HEADER "submit failure");		\
		}																						\
																								\
		g_data->set_purge(work->l2zz.thr_id);													\
		g_data->unlock(work->l2zz.thr_id);														\
		return r;																				\
	} while (0)
...
int lyra2Zz_submit(CURL* curl, struct pool_infos *pool, struct work *work)
{
	if (!lyra2Zz_is_valid_work(work)) {
		applog(LOG_WARNING, LYRA2ZZ_LOG_HEADER "%s", "bad work object passed in.");
		return false;
	}

	g_data->lock(work->l2zz.thr_id);


	l2zz_internal_data *internal_data = (l2zz_internal_data *) work->l2zz.internal_data;

	g_data->internal_log(work->l2zz.thr_id, LYRA2ZZ_LOG_HEADER "Block submit. pool id: %i. l2zz data address: %p work: %p", 
		pool->id, internal_data, work);
	...
	if (!write_transactions_encoded(internal_data, hex_data_stream)) {
...
int lyra2Zz_gbt_work_decode(CURL *curl, const json_t *val, struct work *work)
{
	...
	if (!work->l2zz.needs_internal) {
		applog(LOG_ERR, LYRA2ZZ_LOG_HEADER "%s", "work->l2zz.needs_internal isn't set");
		return false;
	}
	...
	work->l2zz.internal_data = (lyra2zz_internal_t *) internal_data.release();
	work->l2zz.free_fn = l2zz_free_internal_data;
	work->l2zz.deep_copy_fn = l2zz_deep_copy_internal_data;

	{
		g_data->lock(work->l2zz.thr_id);
		g_data->internal_log(
			work->l2zz.thr_id,
			LYRA2ZZ_LOG_HEADER "New work allocated: %p; Work: %p; Pooln: %i", 
			work->l2zz.internal_data, work, work->pooln
		);
		g_data->unlock(work->l2zz.thr_id);
	}
...
int lyra2Zz_is_valid_work(struct work *w)
{
	...
	if (!w->l2zz.needs_internal)
		return false;

	if (!w->l2zz.deep_copy_fn)
		return false;

	if (!w->l2zz.free_fn)
		return false;

	if (w->l2zz.thr_id < 0 || w->l2zz.thr_id >= MAX_GPUS)
		return false;
